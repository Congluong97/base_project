...
import com.pacom.baseproject.utils.setWidth

abstract class BaseDotIndicator @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    styleRes: Int = 0
): FrameLayout(context, attrs, styleRes){
    companion object {
    ...
    enum class Type(
        ...
        DEFAULT(
            16f,
            8f,
            R.styleable.WormDotIndicator,
            R.styleable.WormDotIndicator_dotsColor,
            R.styleable.WormDotIndicator_dotsSize,
            R.styleable.WormDotIndicator_dotsSpacing,
            R.styleable.WormDotIndicator_dotsCornerRadius,
            R.styleable.WormDotIndicator_dotsClickable
        ),
        WORM(
            16f,
            4f,
    ...
    protected var dotsSize: Float = type.defaultSize.dpToPx(context)
    protected var dotsCornerRadius = dotsSize / 2f
    protected var dotsSpacing = type.defaultSpacing.dpToPx(context)
    ...
    fun refreshDots(){
        ...
        post{
            ...
            refreshDotsColors()
            refreshDotsSize()
            refreshOnPageChangeListener()
        }
    }

    private fun refreshOnPageChangeListener(){
        if(pager!!.isNotEmpty){
