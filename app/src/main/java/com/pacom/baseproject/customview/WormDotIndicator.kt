...
import com.pacom.baseproject.utils.getThemePrimaryColor

class WormDotIndicator @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    styleRes: Int = 0
): BaseDotIndicator(context, attrs, styleRes) {
    ...
    init {
        ...
        val horizontalPadding = 24f.dpToPx(context).toInt()
        ...
        dotsStrokeWidth = 2f.dpToPx(context)
        dotIndicatorColor = context.getThemePrimaryColor()
        dotsStrokeColor = dotIndicatorColor

        if (attrs != null) {
            val a = getContext().obtainStyledAttributes(attrs, R.styleable.WormDotIndicator)

            // Dots attributes
            dotIndicatorColor =
                a.getColor(R.styleable.WormDotIndicator_dotsColor, dotIndicatorColor)
            dotsStrokeColor =
                a.getColor(R.styleable.WormDotIndicator_dotsStrokeColor, dotIndicatorColor)

            // Spring dots attributes
            dotsStrokeWidth = a.getDimension(
                R.styleable.WormDotIndicator_dotsStrokeWidth,
                dotsStrokeWidth
            )
